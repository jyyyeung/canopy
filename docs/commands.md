...
| `canopy switch <feature> [--release-current] [--no-evict] [--evict <f>] [--evict-to <slot-N>] [--to-slot <slot-N>]` | **The RUN-target focus primitive (4.0 phase 4).** Promote a feature to the canonical slot — reserved for when you actually need to run it full-stack; addressing review comments happens in the worktree, no switch (see [concepts.md §4](concepts.md#4-the-slot-model)). Validates every in-scope repo before mutating any (no partial flips). Default: the previously-canonical feature goes **warm** iff it has an open PR or live/uncommitted WIP, else **cold** with a feature-tagged stash. `--release-current` forces cold regardless. `--evict <f>` / `--evict-to <slot-N>` override which feature/slot is affected. When the destination is already warm, the swap is a fast 5-op-per-repo dance — no `mv`, no slot renaming. `--to-slot <slot-N>` promotes whatever feature already occupies that slot (omit `<feature>`). Cap-reached (`worktree_cap_reached`) surfaces three explicit fix actions — raise the cap, send the vacating feature cold, or evict a specific warm PR — as a choice, never a silent auto-evict. |
| `canopy reclaim` | **4.0 phase 4.** Free every warm slot whose feature's PR(s) are all merged/closed AND whose worktree is clean: checks out the repo's default branch in the slot and drops the `slots.json` entry, returning the slot to the pool (dir + installed deps persist for the next tenant). A merged-but-dirty slot is left untouched and reported as an advisory instead. Also runs passively inside `canopy context --remote`. |
...
| Command | What it does |
|---|---|
| `canopy worktree <name> [issue]` | Create linked worktrees for a feature in every repo, optionally linking a Linear issue. Worktrees go to `.canopy/worktrees/worktree-N/<repo>/` (generic numbered slot, allocated as the lowest free slot). The allocated slot id is returned in the response. **4.0 phase 4:** slot creation auto-bootstraps — env-file copy, IDE workspace gen, and per-clone hook install (husky `prepare` script, or `.husky/` via `core.hooksPath`) run synchronously so the worktree is immediately usable for edit/commit/push; `install_cmd` runs detached in the background with status (`installing`/`ready`/`failed`) surfaced in `canopy context`. |
| `canopy worktree-bootstrap <feature> [--force] [--step env\|deps\|ide] [--deps] [--interactive]` | **M6, background split in 4.0 phase 4.** Bootstrap a feature's worktrees by hand: env-file copy, `install_cmd`, IDE workspace gen, and the per-clone hook step. `--step` restricts to one step; `--deps` runs deps only (used to retry a `failed`/`installing` slot, or as the target of the background install `canopy context` points at). `--interactive` runs the deps install in the foreground so it can stream output and handle prompts (auth, pnpm build-script approval) that the detached background install can't. `--force` overwrites existing env files / bypasses the lockfile-unchanged short-circuit. |
